...
def search_user_in_ranking(user_id: str, ranked_users: list[dict[str, str | int]]) -> dict[str, str | int | None]:
...
async def handle_qm_point_command(
    ...
    users = assign_ranks(sorted(users, key=lambda x: x["point"], reverse=True))
    reply_text = lang.text("quickmath_points.ranking_title", [
        format_time(int((int(time.time() / 86400) + 1) * 86400 - time.time()))
    ], event.user_id)
    for user in users[:13]:
        reply_text += lang.text("quickmath_points.ranking_item", [
            user["ranking"],
