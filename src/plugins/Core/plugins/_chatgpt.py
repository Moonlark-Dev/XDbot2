        ...
        if argv[0] == "proxy":
            if len(argv) == 1:
                ...
                await matcher.finish(
                    lang.text("chatgpt.proxy_set", [openai.proxy],
                              event.user_id))
        elif argv[0] == "apikey":
            if len(argv) == 1:
                ...
                await matcher.finish(
                    lang.text("chatgpt.apikey_set", [openai.api_key],
                              event.user_id))
...
async def save_data():
    json.dump(messages,
              open("data/chatgpt.messages.json", "w", encoding="utf-8"))
...
async def _(matcher: Matcher,
    ...
    try:
        ...
        await matcher.finish(lang.text("chatgpt.ready", [],
                                       str(event.user_id)))
    except:
        await _error.report()
    ...
    try:
        ...
        if argv[0] in ["clear", "reset"]:
            try:
                ...
                pass
            await matcher.finish(
                lang.text("chatgpt.cache_cleaned", [], event.user_id))
        elif argv[0] == "show":
            reply = ""
