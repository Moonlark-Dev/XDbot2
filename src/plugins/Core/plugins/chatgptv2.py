...
def get_user_info(user_id: str) -> str:
    ...
    return lang.text("chatgpt.user_info",
                     [user_data["token"] or 0, user_data["free"] or 0],
                     user_id)


def init_session(session_id: str,
                 system_message: str | None,
                 force_unlock: bool = False) -> bool:
    if force_unlock:
...
def get_template_by_name(template_name: str) -> str | None:
    try:
        with open(f"src/plugins/Core/chatgpt_template/{template_name}.txt",
                  encoding="utf-8") as f:
            return f.read()
...
def reset_session(session_id: str,
                  ...
                  _system_message: str | None = None) -> bool:
...
async def get_chatgpt_reply(messages: list[dict],
...
def generate_gpt_reply(gpt_reply: str, used_token: int, user_id: str) -> str:
    if used_token > 0:
        token_usage_msg = lang.text(
            "chatgpt.used_token",
            [Json(f"gpt/users/{user_id}.json")["token"], used_token], user_id)
    else:
        token_usage_msg = lang.text(
            "chatgpt.free", [Json(f"gpt/users/{user_id}.json")["free"]],
            user_id)
        ...
        if arguments[0] == "gpt":
            if arguments[1] in ["tokens", "token", "t"]:
                if arguments[2] in ["add"]:
                    ...
                    user_data["token"] = user_data.get("token", 0) + int(
                        arguments[4])
                    await su.finish("完成！")
    except:
        await error.report()
...
async def handle_gpt_command(
    ...
    message: Message = CommandArg()) -> None:
    try:
        ...
        match argv[0]:
            ...
            case "reset":
                ...
                if is_reset_successful:
                    await matcher.finish(
                        lang.text("chatgpt.reset_successful",
                                  [get_user_session(user_id)], user_id))
                else:
                    await matcher.finish(
            ...
            case "switch":
                try:
                    ...
                    await matcher.finish(
                        lang.text("chatgpt.session_not_available", [],
                                  user_id))
                await matcher.finish(
                    lang.text("chatgpt.switched_to_another_session", [argv[1]],
                              user_id))
                ...
                if argv[1] in ["buy", "购买"]:
                    ...
                    if economy.use_vimcoin(user_id, 0.03 * count):
                        Json(f"gpt/users/{user_id}.json")["token"] = Json(
                            f"gpt/users/{user_id}.json").get("token",
                                                             0) + count
                        await send_text("chatgpt.buy_success", [count],
                                        user_id)
                    else:
                        await send_text("currency.no_money", [0.04 * count],
                                        user_id)
                    await matcher.finish()
            case _:
                ...
                if session["is_locked"]:
                    await matcher.finish(
                        lang.text("chatgpt.session_locked", [session_id],
                                  user_id))
                ...
                add_message_to_session(
                    session_id, reply["choices"][0]["message"]["role"],
