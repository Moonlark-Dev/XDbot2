    ...
    try:
        ...
        match argv[0]:
            ...
            case "view" | "":
                ...
                await bank_command.finish(
                    _lang.text("bank.view", [amount_to_be_repaid, debt_list], user_id)
                )
            case "repay":
...
async def _(bot: Bot, event: MessageEvent, message: Message) -> None:
    ...
    match argv[0]:
        case "add":
            data.bank_lead_data[user_id].append({"money": int(argv[2]), "time": time.time()})
        case "give":
            data.bank_lead_data[user_id].append({"money": int(argv[2]), "time": time.time()})
