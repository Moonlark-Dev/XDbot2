...
def view_item(user_id: str, item_id: str) -> str:
    ...
    return lang.text("bag_overflow.item", [
...
async def _(bot: Bot, event: MessageEvent, message: Message):
    ...
    match argv[0]:
        ...
        case "view":
            await finish(
                get_currency_key("empty"),
                [view_item(user_id, argv[1])],
                user_id
            )

        case _:
            await send_node_message(bot, await generate_node_message(bot, [
                view_item(user_id, item_id) for item_id in bag_overflow.get_overflow(user_id).keys()
            ]), event)
