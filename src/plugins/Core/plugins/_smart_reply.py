...
async def create_reply(matcher: str, strings: list[str], group_id: int,
                       user_id: str) -> int:
    data = json.load(open("data/smart_reply.data.json", encoding="utf-8"))
    data[str(data["count"])] = {
        "matcher": matcher,
    ...
    await _error.report((f"「新调教投稿（#{data['count']}）」\n"
                         f"表达式：{matcher}\n"
                         f"文本：{strings}\n"
...
def remove_reply(reply_id: str, user_id: str, force: bool = False) -> bool:
    ...
    if data[reply_id]["user_id"] == user_id or force:
        ...
        json.dump(data,
                  open("data/smart_reply.data.json", "w", encoding="utf-8"))
        if force:
...
def global_reply(reply_id: str) -> None:
    ...
    messenger.send_message(f"您提交的 Reply#{reply_id} 已被全局化",
