...
async def _(bot: Bot, event: MessageEvent, message: Message) -> None:
    ...
    match argv[0]:
        case "group-probability" | "group":
            ...
            if len(argv) == 1:
                await finish(
                    "reply_probability.group_info",
                    [Json("reply/config/group_probability.json").get(group_id, 1)],
                    event.user_id,
                )
            elif (
                event.sender.role not in ["owner", "admin"]
                or event.get_user_id() not in bot.config.superusers
            ):
        ...
        case "":
            await finish(
                "reply_probability.info",
                [Json("reply/config/probability.json").get(event.get_user_id(), 1)],
                event.user_id,
            )
        case _:
