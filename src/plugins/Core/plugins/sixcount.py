...
async def pie():
    ...
    try:
        ...
        start_time = time.strftime(
            ...
            time.localtime(
                json.load(
                    open("data/sixcount.starttime.json",
                         encoding="utf-8"))["time"]),
        )
        ...
        for i in range(len(user_list)):
            ...
            user_data.append((
                (await
                 bot.get_stranger_info(user_id=user_list[i]))["nickname"],
                data[user_list[i]],
            ))

        file_path = (Pie(
            init_opts=opts.InitOpts(bg_color="rgba(255,255,255,1)")).add(
                "", user_data).set_global_opts(
                    title_opts=opts.TitleOpts(title="6",
                                              subtitle=start_time + " 至今"),
                    legend_opts=opts.LegendOpts(
                        is_show=False)).set_series_opts(
                            label_opts=opts.LabelOpts(
                                formatter="{b}: {c}"))).render(
                                    path="data/sixcount.render.ro.html")
...
async def pie():
    ...
    try:
        ...
        for i in range(len(user_list)):
            ...
            if len(user_list) >= 20 and data[
                    user_list[i]] <= retention_threshold:
        ...
        if other_count > 0:
            user_data.append(("Other", other_count))
