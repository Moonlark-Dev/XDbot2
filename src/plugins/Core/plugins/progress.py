...
def calculate_percentage_of_year() -> float:
    ...
    if current_datetime.year % 4 == 0 and (
        current_datetime.year % 100 != 0 or current_datetime.year % 400 == 0
    ):
...
def calculate_percentage_of_month() -> float:
    ...
    if current_datetime.month == 12:
        ...
        next_month = datetime.datetime(
            current_datetime.year, current_datetime.month + 1, 1
        )
...
async def _(bot: Bot, event: MessageEvent, message: Message) -> None:
    await finish(
        "progress.year",
        [datetime.datetime.now().year, calculate_percentage_of_year()],
        event.user_id,
    )
