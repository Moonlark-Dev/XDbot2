...
async def _(event: GroupMessageEvent, message: Message = CommandArg()):
    try:
        ...
        if args[0] in ["give", "给"]:
            bag.add_item(
                args[1].replace("[CQ:at,qq=", "").replace("]", ""), args[2],
...
async def update_xdbot(matcher: Matcher, event: MessageEvent):
    try:
        await matcher.send(
            lang.text("update.checking", [], event.get_user_id()))
        old_commit = os.popen("git log").read().split("\n")[0].split(
            " ")[1][:7]
        os.system("python3 update.py")
        await matcher.finish(
            lang.text("update.finish", [
                old_commit,
...
@on_command("checkout",
            permission=GROUP_ADMIN | GROUP_OWNER | SUPERUSER).handle()
async def checkout_xdbot(matcher: Matcher,
                         event: GroupMessageEvent,
                         node: Message = CommandArg()):
    try:
        if node.extract_plain_text() == "develop":
            Json("node_manager.groups.json")[str(event.group_id)] = True
            await matcher.finish(
                lang.text("node_manager.enabled", [], event.user_id))
        else:
            Json("node_manager.groups.json")[str(event.group_id)] = None
            await matcher.finish(
                lang.text("node_manager.disabled", [], event.user_id))
    except:
        await error.report()
