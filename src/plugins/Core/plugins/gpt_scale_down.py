...
async def scale_session(session_id: str) -> str | None:
    ...
    memory = (await get_chatgpt_reply(get_session_messages(session_id))).choices[0].message.content
...
async def _(bot: Bot, event: MessageEvent, message: Message, matcher: Matcher = Matcher()) -> None:
