...
class Weapons(Item):
    ...
    def _after_register(self):
        self.data['display_name'] = (kit_data := load_json(f"kits/{self.data['kit']}.json"))["weapons"]["name"]
...
class Ball(Item):
    ...
    def _after_register(self):
        self.data['display_name'] = (kit_data := load_json(f"kits/{self.data['kit']}.json"))["ball"]["name"]
        self.data["display_message"] = (
            f"所属套装：{kit_data['name']}\n"
            "面位球（暂无说明）"
        )
