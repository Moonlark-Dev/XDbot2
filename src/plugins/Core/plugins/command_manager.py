...
async def block_blocked_command(event: GroupMessageEvent) -> None:
    if (
        event.get_user_id() in get_driver().config.superusers
        or await _group_admin(event)
        or await _group_owner(event)
    ):
        return
    if is_msg_call_blocked_cmd(
        event.get_plaintext(), get_blocked_commands(event.group_id)
    ):
...
def can_block_command(command_name: str, group_id: int) -> bool:
    return (not is_command_blocked(command_name, group_id)) and is_command_valid(
        command_name
    )
...
def unblock_command(command_name: str, group_id: int):
    if not can_unblock_command(command_name, group_id):
        return FAILED
    blocked_commands: list = Json(f"command_manager/{group_id}.json").get(
        "blocked_command", []
    )
...
@on_command(
    "cm",
    aliases={"command-manager", "command"},
    permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER,
).handle()
async def handle_cm_command(
    ...
    try:
        ...
        match argv[0]:
            ...
            case "list" | "":
                await finish(
                    "cm.blocked_command_list",
                    [
                        " ".join(
                            Json(f"command_manager/{event.group_id}.json").get(
                                "blocked_command", []
                            )
                        )
                    ],
