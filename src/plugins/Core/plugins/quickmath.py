...
def render_question(question_string: str) -> bytes:
    title_font = ImageFont.truetype(
        "./src/plugins/Core/font/HYRunYuan-55W.ttf", 17)
    question_font = ImageFont.truetype(
        "./src/plugins/Core/font/HYRunYuan-55W.ttf", 20)
...
def get_group() -> int:
    try:
        return random.choice(json.load(open("data/quick_math.enabled_groups.json", encoding="utf-8")))
    except BaseException:
        return 0

def check_group(group_id: int) -> bool:
    return bool(group_id and Json("quickmath/group_unanswered.json").get(str(group_id), 0) <= 3)
...
async def send_quick_math():
    try:
        ...
        msg_id = (await bot.send_group_msg(
            group_id=group,
            message=MessageSegment.image(render_question(question)),
        ))["message_id"]
        ...
        async def handle_quickmath_answer(event: GroupMessageEvent):
            nonlocal answered
            try:
                await send_text("quick_math.rightanswer", [(add_score := 2 * (time.time() - send_time))], evnet.user_id, True, True)
                matcher.destroy()
                answered = True
            except:
                await error.report()
