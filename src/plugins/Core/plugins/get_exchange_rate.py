...
get_exchange_rate = on_command("getexchangerate", aliases={
                               "ger", "获取当前汇率", "获取汇率"})
...
async def handler(event: MessageEvent):
    try:
        ...
        await get_exchange_rate.finish("\n".join((
            lang.text("ger.title", [time.strftime(
                "%H:%M:%S", time.localtime())], qq),
            lang.text("ger.io", [economy.vimcoin["in"],
                      economy.vimcoin["out"]], qq),
