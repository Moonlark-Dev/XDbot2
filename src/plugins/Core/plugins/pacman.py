...
pacman = on_command("archpackage",
                    aliases={"apkg", "pacman", "Linux搜包", "spkg", "pkg"})
...
def get_packages_list(html: str):
    return html[html.find("<tbody>") +
                7:html.find("</tbody>", html.find("<tbody>"))].replace(
                    "<tr>",
                    "").replace("<td>",
                                "").replace("\n",
                                            "").replace("</a>",
...
def parse_package_data(package: str):
    ...
    try:
        return {
            ...
            package_data[1],
            "name":
            package_data[2][package_data[2].find(">") + 1:],
            "ver":
            package_data[3],
            "info":
            package_data[4][package_data[4].find(">") + 1:],
            "latest_update":
            package_data[5],
            "url":
            f"https://archlinux.org/packages/{package_data[1].lower()}/{package_data[0]}/{package_data[2][package_data[2].find('>')+1:]}"
        }
    except IndexError:
        return None
...
async def search_package(event: MessageEvent, message: Message = CommandArg()):
    if str(message) == "":
        await pacman.finish(
            lang.text("pacman.need_pkg_name", [], event.user_id))
    try:
        packages = parse_packages_data(
            get_packages_list(await send_request(process_input(str(message)))))
        for package in packages:
            await pacman.send(
                lang.text("pacman.pkg_info", [
                    package['name'], package['ver'], package['arch'],
                    package['info'], package['latest_update'], package['url']
                ], event.user_id))
        await pacman.finish()
