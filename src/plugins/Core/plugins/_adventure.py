...
def compute_user_to_point_distance(
    user_id: str | int, target: tuple[int, int]
) -> float:
...
def get_block_id(pos: tuple[int, int]) -> str:
    return "_".join([str(int((pos[0] + 16) / 32)), str(int((pos[1] + 16) / 32))])
...
def get_block_data(block_id: str) -> Json:
    ...
    if not data.to_dict():
        data.update(
            {"center": get_block_center(block_id), "items": [], "protected": False}
        )
    return data
...
def get_outer_blocks(center: tuple[int, int], radius: int) -> set:
    center_block = cast(
        tuple[int, int], tuple([int(i) for i in get_block_id(center).split("_")])
    )
    blocks = set()
...
def find_teleportation_point(
    user_id: str | int, pos: tuple[int, int], max_distance: int
):
        ...
        for block_id in get_outer_blocks(pos, radius):
            ...
            for item in block["items"]:
                ...
                if item["type"].endswith("teleportation_point"):
                    # TODO 权限校验
                    if (not nearest_teleportation_point) or calculate_distance(
                        nearest_teleportation_point["pos"], pos
