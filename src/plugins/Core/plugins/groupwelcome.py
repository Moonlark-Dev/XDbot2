...
async def getAcceptedQQList():
    async with httpx.AsyncClient() as client:
        r = (await client.get(
            'https://api.github.com/repos/ITCraftDevelopmentTeam/Forum/issues/14/comments'
        )).json()
...
defaultConfig = {
    ...
    "welcome_message":
    "欢迎加入本群，我是由 IT Craft Development Team 开发的机器人 XDbot2，使用 /help 查看命令帮助",
...
try:
    ...
    json.dump(defaultConfig,
              open("data/groupwelcome.config.json", "w", encoding="utf-8"))


def writeConfig(cfg):
    json.dump(cfg, open("data/groupwelcome.config.json", "w",
                        encoding="utf-8"))
        ...
        if args == "":
            if event.group_id in config["welcome_enabled_group"]:
                ...
                await configCommand.finish(
                    _lang.text("groupwelcome.group.disabled", [],
                               event.get_user_id))
            else:
                config["welcome_enabled_group"].append(event.group_id)
                writeConfig(config)
                await configCommand.finish(
                    _lang.text("groupwelcome.group.enabled", [],
                               event.get_user_id))
        else:
            args = args.split(" ")
            if args[0] == "global":
                if config["welcome_global_enabled"]:
                    ...
                    writeConfig(config)
                    await configCommand.finish(
                        _lang.text("groupwelcome.global.disabled", [],
                                   event.get_user_id))
                else:
                    config["welcome_global_enabled"] = True
                    writeConfig(config)
                    await configCommand.finish(
                        _lang.text("groupwelcome.global.enabled", [],
                                   event.get_user_id))
            elif args[0] == "application":
                if event.group_id in config["application_check_enabled_group"]:
                    config["application_check_enabled_group"].remove(
                        event.group_id)
                    writeConfig(config)
                    await configCommand.finish(
                        _lang.text("groupwelcome.application.disabled", [],
                                   event.get_user_id))
                else:
                    config["application_check_enabled_group"].append(
                        event.group_id)
                    writeConfig(config)
                    await configCommand.finish(
                        _lang.text("groupwelcome.application.enabled", [],
                                   event.get_user_id))
            elif args[0] in ["welcome_message", "wm"]:
                config["welcome_message"] = args[1]
                writeConfig(config)
                await configCommand.finish(
                    _lang.text("groupwelcome.message.wm.edited", [args[1]],
                               event.get_user_id))
            elif args[0] in ["application_approve_message", "aam"]:
                config["application_approve_message"] = args[1]
                writeConfig(config)
                await configCommand.finish(
                    _lang.text("groupwelcome.message.aam.edited", [args[1]],
                               event.get_user_id))
            elif args[0] in ["application_notfound_message", "anm"]:
                config["application_notfound_message"] = args[1]
                writeConfig(config)
                await configCommand.finish(
                    _lang.text("groupwelcome.message.anm.edited", [args[1]],
                               event.get_user_id))
            elif args[0] == "confirm_reset_config":
                writeConfig(defaultConfig)
                await configCommand.finish(
                    _lang.text("groupwelcome.config.reset", [],
                               event.get_user_id))
...
async def _(bot: Bot, event: GroupIncreaseNoticeEvent, matcher: Matcher):
    try:
        if event.group_id in config["welcome_enabled_group"] or config[
                "welcome_global_enabled"]:
        ...
        if event.group_id in config["application_check_enabled_group"]:
            if str(event.user_id) in await getAcceptedQQList():
                await GroupIncrease.finish(
                    config["application_approve_message"], at_sender=True)
            else:
                await GroupIncrease.finish(
