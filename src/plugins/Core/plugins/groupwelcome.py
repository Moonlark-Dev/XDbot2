...
try:
    ...
    json.dump(
        defaultConfig, open("data/groupwelcome.config.json", "w", encoding="utf-8")
    )
        ...
        if args == "":
            if event.group_id in config["welcome_enabled_group"]:
                ...
                await configCommand.finish(
                    _lang.text("groupwelcome.group.disabled", [], event.get_user_id)
                )
            else:
                config["welcome_enabled_group"].append(event.group_id)
                writeConfig(config)
                await configCommand.finish(
                    _lang.text("groupwelcome.group.enabled", [], event.get_user_id)
                )
        else:
            args = args.split(" ")
            if args[0] == "global":
                if config["welcome_global_enabled"]:
                    ...
                    await configCommand.finish(
                        _lang.text(
                            "groupwelcome.global.disabled", [], event.get_user_id
                        )
                    )
                else:
                    ...
                    writeConfig(config)
                    await configCommand.finish(
                        _lang.text("groupwelcome.global.enabled", [], event.get_user_id)
                    )
                ...
                if event.group_id in config["application_check_enabled_group"]:
                    ...
                    await configCommand.finish(
                        _lang.text(
                            "groupwelcome.application.disabled", [], event.get_user_id
                        )
                    )
                else:
                    ...
                    await configCommand.finish(
                        _lang.text(
                            "groupwelcome.application.enabled", [], event.get_user_id
                        )
                    )
                ...
                await configCommand.finish(
                    _lang.text(
                        "groupwelcome.message.wm.edited", [args[1]], event.get_user_id
                    )
                )
                ...
                await configCommand.finish(
                    _lang.text(
                        "groupwelcome.message.aam.edited", [args[1]], event.get_user_id
                    )
                )
                ...
                await configCommand.finish(
                    _lang.text(
                        "groupwelcome.message.anm.edited", [args[1]], event.get_user_id
                    )
                )
            elif args[0] == "confirm_reset_config":
                writeConfig(defaultConfig)
                await configCommand.finish(
                    _lang.text("groupwelcome.config.reset", [], event.get_user_id)
                )
