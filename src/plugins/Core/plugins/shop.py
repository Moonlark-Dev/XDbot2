    ...
    try:
        ...
        if argument[0] == "":
            ...
            for item in list(shopData.keys()):
                text += (
                    f"{item}. {shopData[item]['name']} ({shopData[item]['price']} vi)\n"
                )
...
+-------------------------+"""
            ...
            if count > itemData["count"]:
                await shop.finish(
                    _lang.text("shop.not_enough", [], event.get_user_id()),
                    at_sender=True,
                )
            # 购买
            if _userCtrl.removeItemsByID(
                ...
                _userCtrl.addItem(
                    event.get_user_id(), itemData["item"]["id"], count, itemData["item"]
                )
                _userCtrl.addItem(
                ...
                await shop.finish(
                    _lang.text("shop.buy_success", [], event.get_user_id()),
                    at_sender=True,
                )
            else:
                await shop.finish(
                    _lang.text("shop.buy_failed", [], event.get_user_id()),
                    at_sender=True,
                )
            ...
            try:
                _userCtrl.removeItemsFromBag(
                    event.get_user_id(), int(argument[1]), int(argument[2]), "Sell"
                )
            except _userCtrl.NotHaveEnoughItem:
                await shop.finish(
                    _lang.text("shop.sell_not_enough", [], event.get_user_id())
                )
            except _userCtrl.ItemCanNotRemove:
                await shop.finish(
                    _lang.text("shop.sell_cannot_remove", [], event.get_user_id())
                )
            itemData = {
            ...
            await bot.send_group_msg(
                message=_lang.text(
                    "shop.sell_success1", [length, itemData], event.get_user_id()
                ),
                group_id=ctrlGroup,
            )
            await shop.finish(
                _lang.text("shop.sell_success2", [length], event.get_user_id()),
                at_sender=True,
            )
