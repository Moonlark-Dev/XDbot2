...
async def init_duel(bot: Bot, active_user_id: int, passive_user_id: int) -> Scheduler:
    active_contingent = Contingent([
        await init_monomer(bot, active_user_id)
    ])
    passive_contingent = Contingent([
        await init_monomer(bot, passive_user_id)
    ])
...
async def handle_duel_command(bot, event: GroupMessageEvent, message: Message):
    ...
    # TODO 检查是否装备装备
    scheduler = await init_duel(bot, duel_requests[event.user_id]["active"], event.user_id)
